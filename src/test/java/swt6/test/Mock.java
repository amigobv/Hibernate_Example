...
public class Mock {
			...
			if (pEmpl[i] != null) {
				for (int j = 0; j < NUM_OF_LOGBOOKS; j++) {
					LogbookEntry entry = new LogbookEntry(ACTIVITIES[j],
							...
							DateUtil.getTime(14 + j, 30 + i), phase, module);
					pEmpl[i] = Dao.addLogbookEntry(pEmpl[i], entry);
				}
			}
			...
			if (tEmpl[i] != null) {
				for (int j = 0; j < NUM_OF_LOGBOOKS; j++) {
					LogbookEntry logbook = new LogbookEntry(ACTIVITIES[j],
							...
							DateUtil.getTime(15 + j, 30), phase, module);
					tEmpl[i] = Dao.addLogbookEntry(tEmpl[i], logbook);
				}
			}
		}
	}
	...
	private static void CleanLogbooks() {
		List<LogbookEntry> logbooks = Dao.getAllLogbooks();
		
		for (LogbookEntry entry : logbooks) {
	...
	private static void CleanPhases() {
		if (phase != null)
			Dao.removeItem(Phase.class, phase.getId());
	}

	private static void CleanModules() {
		if (module != null) {
			module.detachProject();
			module.getLogbooks().clear();	
		}
	}
	...
	private static void CleanEmployees() {
		for (int i = 0; i < NUM_OF_EMPLOYEES; i++) {
			...
			if (pEmpl[i] != null) {
				Dao.removeEmployee(pEmpl[i].getId());
			}
		}
	}
}
